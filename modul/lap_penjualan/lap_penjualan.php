...
$(document).ready(function() {
	$("#data").load('modul/lap_penjualan/tampil_data.php');
	...
	$("#cari").click(function() {
		...
		if (kode2.length==''){
	   	...
	   	$.ajax({
			type	: "GET",
			url		: "modul/lap_penjualan/tampil_data.php",
	...
	$("#cetak").click(function() {
		var kode1	= $("#kode1").val();
		var kode2	= $("#kode2").val();
		window.location.href='modul/laporan/lap_penjualan.php?kode1='+kode1+'&kode2='+kode2;
		/*
	   	$.ajax({
			type	: "GET",
			url		: "modul/laporan/lap_stok_penjualan.php",
			data	: "kode1="+kode1+"&kode2="+kode2,
			success	: function(data){
				//$("#data").html(data);
	...
	<fieldset>
		...
		<table width='100%'>
			<tr>
				...
				<option value=''>-Pilih Tanggal Jual-</option>";
				$query	= "SELECT * FROM penjualan ORDER BY tgl_jual";
				$sql	= mysql_query($query);
				while($r_data=mysql_fetch_array($sql)){
					echo "<option value='".$r_data['tgl_jual']."'>".$r_data['tgl_jual']." - ".$r_data['nama_penjualan']."</option>";
				}
				echo "</select>
				</td>
